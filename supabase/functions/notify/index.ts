...
function handleError(error) {
  console.error(error);
  return new Response(`Edge function (Notify) error: ${error.message}`, {
    status: 400,
    headers: {
...
Deno.serve(async (req)=>{
  try {
    ...
    if (!record) {
      return new Response('No record payload', {
        status: 400,
        headers: {
          'Content-Type': 'application/json'
        }
    ...
    if (record.owner_type !== 'character') {
      return new Response(JSON.stringify({
        message: 'Owner type is not character, notifications skipped'
      }), {
        status: 200,
        headers: {
          'Content-Type': 'application/json'
    ...
    const { data: notificationData, error: notificationError } = await supabase.rpc('get_notification_data', {
      post_id: record.id
    });
    if (notificationError) {
      return handleError(notificationError);
    }
    ...
    if (notificationData.length > 0) {
      ...
      if (characterError) {
        return handleError(characterError);
      }
      if (!character) {
        console.log('Character not found for owner:', record.owner);
        return new Response(JSON.stringify({
          message: 'Character missing, notifications skipped'
        }), {
          status: 200,
          headers: {
            'Content-Type': 'application/json'
