...
create table games (
  ...
  annotation text,
  recruitment text default 'Informace o náboru nových hráčů, četnosti hraní a tvorbě postav.'::text,
  prompt text default 'Z tohoto textu vychází AI vypravěč pro tvorbu příběhu.'::text,
  story text,
  notes text default 'Poznámky a zápisky ke hře, záznamy o hraní, plány apod.'::text,
...
create or replace function create_profile (id uuid, name text, old_id int4, created_at timestamp with time zone DEFAULT current_timestamp) returns void as $$
begin
  insert into profiles (id, name, old_id, created_at) values (id, name, old_id, created_at);
end;
