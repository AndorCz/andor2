...
drop view if exists wall_posts;
drop view if exists news_reactions;
...
create table solo_concepts (
  ...
  score_total int4 not null default 0,
  score_count int4 not null default 0,
  score_avg numeric(3,2) not null default 0,
...
create table wall (
  id int4 not null primary key generated by default as identity,
  entry_type text not null default 'thread',
  post int4[],
  game_id int4,
  solo_concept_id int4,
  work_id int4,
  board_id int4,
  title text,
  summary text,
  ...
  constraint wall_entry_shape_check check (
    ...
    (
      ...
      and post is null
      and owner is null
      and num_nonnulls(game_id, solo_concept_id, work_id, board_id) = 1
...
create table poll_votes (
  ...
  created_at timestamp with time zone default current_timestamp,
  constraint poll_votes_profile_fkey foreign key (profile_id) references profiles(id) on delete cascade,
...
create or replace view news_reactions with (security_invoker = true) as
  ...
  from news n
  left join reactions r on n.id = r.item_id and r.item_type = 'news'
  left join profiles p on n.owner = p.id;

grant select on public.news to anon, authenticated;
grant select on public.news_reactions to anon, authenticated;


create or replace view wall_posts with (security_invoker = true) as
  select
    w.id as wall_id,
    w.owner as wall_owner,
    w.post,
    wall_position.position::int,
    w.published,
    w.created_at as wall_created_at,
    root.*,
    case
      when root.owner_type = 'user' then profile_owner.name
      when root.owner_type = 'character' then character_owner.name
      when root.owner_type = 'npc' then npc_owner.name
    end as owner_name,
    case
      when root.owner_type = 'user' then profile_owner.portrait
      when root.owner_type = 'character' then character_owner.portrait
      when root.owner_type = 'npc' then npc_owner.portrait
    end as owner_portrait,
    (select array_agg(character.name order by character.name) from unnest(root.audience) audience_id join characters character on character.id = audience_id) as audience_names,
    case when root.owner_type = 'user' then profile_owner.reward_icon end as owner_reward_icon,
    case when root.owner_type = 'user' then profile_owner.reward_link end as owner_reward_link,
    r.thumbs,
    r.frowns,
    r.shocks,
    r.hearts,
  ...
  left join profiles profile_owner on root.owner = profile_owner.id and root.owner_type = 'user'
...
grant update (post) on public.wall to authenticated;
grant usage, select on sequence public.wall_id_seq to authenticated;
...
create or replace function validate_wall_posts () returns trigger as $$
declare
  appended_post_id int4;
begin
  if new.entry_type <> 'thread' then return new; end if;

  if cardinality(new.post) = 0 or (
    select count(*) from posts where id = any(new.post)
  ) <> cardinality(new.post) then
    raise exception 'Wall musí obsahovat neprázdné pole existujících příspěvků';
  end if;

  if tg_op = 'UPDATE' and current_user = 'authenticated' and pg_trigger_depth() = 1 and new.post is distinct from old.post then
    if cardinality(new.post) <> cardinality(old.post) + 1
      or new.post[1:cardinality(old.post)] is distinct from old.post then
      raise exception 'Do vlákna lze přidat pouze jeden nový příspěvek na konec';
    end if;

    appended_post_id := new.post[cardinality(new.post)];
    if not exists (
      select 1 from posts
      where id = appended_post_id
        and owner = (select auth.uid())
        and owner_type = 'user'
        and thread is null
    ) or exists (
      select 1 from wall
      where appended_post_id = any(post)
    ) then
      raise exception 'Do vlákna lze přidat pouze vlastní příspěvek';
    end if;
  end if;
  return new;
end;
...
create or replace function remove_post_from_wall () returns trigger as $$
begin
  delete from wall where old.id = any(post) and cardinality(post) = 1;
  update wall set post = array_remove(post, old.id) where old.id = any(post);
  return old;
end;
$$ language plpgsql set search_path = public;


create or replace function create_wall_entry (new_content text, is_published boolean default true) returns int4 as $$
declare
  new_post_id int4;
  new_wall_id int4;
begin
  if auth.uid() is null then raise exception 'Pro přidání příspěvku se musíš přihlásit'; end if;
  if length(trim(new_content)) = 0 then raise exception 'Příspěvek nesmí být prázdný'; end if;

  insert into posts (owner, owner_type, content)
  values (auth.uid(), 'user', new_content)
  returning id into new_post_id;

  insert into wall (post, published, owner)
...
create or replace function create_wall_reply (target_wall_id int4, new_content text) returns int4 as $$
declare
  new_post_id int4;
begin
  if auth.uid() is null then raise exception 'Pro přidání reakce se musíš přihlásit'; end if;
  if length(trim(coalesce(new_content, ''))) = 0 then raise exception 'Příspěvek nesmí být prázdný'; end if;
  ...
  update wall
  set post = array_append(post, new_post_id)
  where id = target_wall_id and entry_type = 'thread' and published = true;

  if not found then raise exception 'Vlákno na zdi neexistuje'; end if;
  return new_post_id;
end;
$$ language plpgsql security invoker set search_path = public;


create or replace function update_wall_entry (wall_id int4, new_content text) returns void as $$
declare
  root_post_id int4;
begin
  if auth.uid() is null then raise exception 'Pro úpravu příspěvku se musíš přihlásit'; end if;
  if length(trim(new_content)) = 0 then raise exception 'Příspěvek nesmí být prázdný'; end if;

  select post[1] into root_post_id from wall where id = wall_id and owner = auth.uid();
  if root_post_id is null then raise exception 'Příspěvek na zdi neexistuje'; end if;
  update posts set content = new_content where id = root_post_id and owner = auth.uid();
end;
$$ language plpgsql security invoker set search_path = public;


create or replace function delete_wall_entry (wall_id int4) returns void as $$
declare
  post_ids int4[];
begin
  if auth.uid() is null then raise exception 'Pro smazání příspěvku se musíš přihlásit'; end if;

  delete from wall where id = wall_id and owner = auth.uid() returning post into post_ids;
  if post_ids is null then raise exception 'Příspěvek na zdi neexistuje'; end if;
  delete from posts where id = any(post_ids) and owner = auth.uid();
end;
$$ language plpgsql security invoker set search_path = public;

revoke execute on function create_wall_entry(text, boolean) from public, anon;
revoke execute on function create_wall_reply(int4, text) from public, anon;
revoke execute on function update_wall_entry(int4, text) from public, anon;
revoke execute on function delete_wall_entry(int4) from public, anon;
grant execute on function create_wall_entry(text, boolean) to authenticated;
grant execute on function create_wall_reply(int4, text) to authenticated;
grant execute on function update_wall_entry(int4, text) to authenticated;
...
begin
  source_column := case tg_table_name
    ...
    when 'solo_concepts' then 'solo_concept_id'
    when 'works' then 'work_id'
    when 'boards' then 'board_id'
  end;
...
CREATE UNIQUE INDEX wall_board_id_key ON public.wall USING btree (board_id) WHERE board_id IS NOT NULL;
