...
create or replace view wall_posts with (security_invoker = true) as
  select
    ...
    r.laughs
  from wall w
  join posts root on root.id = w.post[1]
  left join profiles author on w.owner = author.id
...
declare
  ...
  new_wall_id int4;
begin
  if not is_admin() then raise exception 'Příspěvky na zeď může přidávat jen administrátor'; end if;
...
declare
  root_post_id int4;
begin
  if not is_admin() then raise exception 'Příspěvky na zdi může upravovat jen administrátor'; end if;
  if length(trim(new_content)) = 0 then raise exception 'Příspěvek nesmí být prázdný'; end if;

  select post[1] into root_post_id from wall where id = wall_id;
  if root_post_id is null then raise exception 'Příspěvek na zdi neexistuje'; end if;
  update posts set content = new_content where id = root_post_id;
end;
$$ language plpgsql security definer set search_path = public;
...
declare
  post_ids int4[];
begin
  if not is_admin() then raise exception 'Příspěvky ze zdi může mazat jen administrátor'; end if;

  delete from wall where id = wall_id returning post into post_ids;
  if post_ids is null then raise exception 'Příspěvek na zdi neexistuje'; end if;
  delete from posts where id = any(post_ids);
end;
$$ language plpgsql security definer set search_path = public;
