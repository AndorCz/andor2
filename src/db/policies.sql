...
alter table public.posts enable row level security; -- (0/8)

create policy "UPDATE, DELETE for owners" on public.posts for update, delete to authenticated using owner = auth.uid();
create policy "INSERT for players or users" on public.posts for insert to authenticated using ( thread in ( select thread from boards union select thread from works union select discussion_thread as thread from games where is_player(id) union select game_thread as thread from games where where is_player(id) ) );
  -- games
