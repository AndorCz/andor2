  ...
  return new Promise((resolve, reject) => {
    const intervalId = setInterval(async () => {
      try {
        ...
        if (status !== 'completed') { reject(new Error('Operation failed')) }
        else { resolve(returnLastMessage ? (await openai.beta.threads.messages.list(threadId)).data[0].content[0].text.value : undefined) }
      } catch (error) {
        clearInterval(intervalId)
        reject(error)
      }
