...
async function migrateOldPosts (oldGameId, newGameId, newGameThread, idMap, locals) {
  ...
  await locals.supabase
    .from('old_games')
    .update({ migrating: false, mihrated: true })
...
async function createGame (locals, oldGameData) {
  ...
  // migrate posts
  await migrateOldPosts(oldGameData.id_game, newGameId, newGameThread, idMap, locals)
}
