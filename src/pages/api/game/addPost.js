...
export const POST = async ({ request, redirect, locals }) => {
  ...
  if (locals.user.id) { // check if user is signed in
    // save to db  
    const { error } = await supabase.from('posts').insert({ thread: data.thread, owner: data.character, owner_type: 'character', content: data.content })
    if (error) { return new Response(JSON.stringify({ error: error.message }), { status: 500 }) }

    // send to openai thread
    // savePost(data.thread, data.content, data.character)
