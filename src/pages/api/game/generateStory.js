...
export const GET = async ({ request, url, locals }) => {
  ...
  function processEvent (controller, encoder, data, responses) {
  ...
  if (storytellerData) {
    ...
    const readable = new ReadableStream({
      async start (controller) {
        ...
        if (data) { processEvent(controller, encoder, data, responses, locals) } else { return 'Failed to generate the story (setting)' }
        ...
        data = await processRun(threadId, assistantId, true)
        if (data) { processEvent(controller, encoder, data, responses, locals) } else { return 'Failed to generate the story (factions)' }
        ...
        if (res.error) { return console.error(res.error) }
        data = await processRun(threadId, assistantId, true)
        if (data) { processEvent(controller, encoder, data, responses, locals) } else { return 'Failed to generate the story (characters)' }
        ...
        if (res.error) { return console.error(res.error) }
        data = await processRun(threadId, assistantId, true)
        if (data) { processEvent(controller, encoder, data, responses, locals) } else { return 'Failed to generate the story (locations)' }
        ...
        if (res.error) { return console.error(res.error) }
        data = await processRun(threadId, assistantId, true)
        if (data) { processEvent(controller, encoder, data, responses, locals) } else { return 'Failed to generate the story (plot)' }
